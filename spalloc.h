...
//			assert(-1 != item_no && "iterator is already at the end - and can't ++");
	...
	private: // iterative members
		typename clusters_t::iterator cluster_it;
		unsigned item_no;					// in cluster_t: 0 <= .. < cluster.size()
	private: // const props of the class
		const typename clusters_t::const_iterator endIt;

		typed_iterator(unsigned query_nbr, typename clusters_t::iterator sit,
			typename clusters_t::iterator end_it) :
...
//			assert(di->type_hash == typeid(Tn).hash_code() && "Incosistent data error");
	...
	typed_iterator<Tt> query_type(unsigned query_no = 0) {
		auto i = std::find(m_kinds.begin(), m_kinds.end(),
	...
	template <typename Tn>
	void go_to_cluster(cluster_iterator<Tn> &it, dataitem_t<N> *obj) {
